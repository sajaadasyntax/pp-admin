...
export default function DistrictsPage() {
  ...
  const [selectedDistrictForSectors, setSelectedDistrictForSectors] = useState<District | null>(null);
  ...
  const fetchSectorsForDistrict = async (district: District) => {
    if (!token) return;
    
    setLoadingSectors(true);
    try {
      const response = await fetch(`${apiUrl}/sector-hierarchy/sector-districts?originalOnly=true`, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
        },
  ...
  const fetchSectorMembersForDistrict = async (sectorId: string) => {
    if (!token) return;

    setLoadingSectorMembers(true);
    try {
      const response = await fetch(`${apiUrl}/sector-hierarchy/members/district/${sectorId}`, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
        },
  ...
  const fetchAvailableSectorUsersForDistrict = async (sectorId: string) => {
    if (!token) return;

    try {
      const response = await fetch(`${apiUrl}/sector-hierarchy/available-users/district/${sectorId}`, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
        },
  ...
  const handleManageSectors = (district: District) => {
    setSelectedDistrictForSectors(district);
  ...
  const handleSelectSectorForDistrict = (sector: Sector) => {
    setSelectedSector(sector);
    setSectorSearchQuery('');
    fetchSectorMembersForDistrict(sector.id);
    fetchAvailableSectorUsersForDistrict(sector.id);
  };

  const handleAddUserToSectorForDistrict = async (userId: string) => {
    if (!token || !selectedSector) return;

    setSubmittingSector(true);
    try {
      const response = await fetch(`${apiUrl}/sector-hierarchy/members/district/${selectedSector.id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
      ...
      if (response.ok) {
        fetchSectorMembersForDistrict(selectedSector.id);
        fetchAvailableSectorUsersForDistrict(selectedSector.id);
      } else {
  ...
  const handleRemoveUserFromSectorForDistrict = async (userId: string) => {
    if (!token || !selectedSector) return;

    const member = sectorMembers.find(m => m.id === userId);
    const memberName = getSectorMemberName(member);
    ...
    setSubmittingSector(true);
    try {
      const response = await fetch(`${apiUrl}/sector-hierarchy/members/district/${selectedSector.id}/${userId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const getSectorMemberName = (member?: SectorMember): string => {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {districts.map((district) => (
            ...
            >
              ...
              <div className="flex gap-2 mb-2">
                <button
                  onClick={() => handleManageSectors(district)}
                  className="flex-1 px-3 py-2 bg-teal-50 text-teal-700 rounded-lg hover:bg-teal-100 text-xs font-medium"
                >
                  🏛️ إدارة القطاعات
                </button>
              </div>
      ...
      {showSectorModal && selectedDistrictForSectors && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-bold">إدارة القطاعات - {selectedDistrictForSectors.name}</h2>
                <button
                  onClick={() => {
                    setShowSectorModal(false);
                    setSelectedDistrictForSectors(null);
                    setSelectedSector(null);
                    setSectors([]);
                    setSectorMembers([]);
                    setAvailableSectorUsers([]);
                  ...
                  <div className="space-y-3">
                    ...
                    {FIXED_SECTOR_TYPES.map((type) => {
                      ...
                      return (
                        <button
                          key={type}
                          onClick={() => sector && handleSelectSectorForDistrict(sector)}
                          disabled={!sector}
                    ...
                    {sectors.length === 0 && (
                      <div className="text-center py-4 text-gray-500 text-sm">
                        <p>لم يتم إنشاء قطاعات لهذا الحي بعد</p>
                          ...
                          {loadingSectorMembers ? (
                            ...
                            <div className="space-y-2 max-h-40 overflow-y-auto">
                              {sectorMembers.map((member) => (
                                ...
                                >
                                  <div>
                                    <div className="font-medium text-sm">{getSectorMemberName(member)}</div>
                                    <div className="text-xs text-gray-500">{member.mobileNumber}</div>
                                  </div>
                                  <button
                                    onClick={() => handleRemoveUserFromSectorForDistrict(member.id)}
                        ...
                        <div>
                          ...
                          <div className="space-y-2 max-h-40 overflow-y-auto">
                            {availableSectorUsers
                              .filter(u => 
                                !sectorSearchQuery || 
                                getSectorMemberName(u).toLowerCase().includes(sectorSearchQuery.toLowerCase()) ||
                              ...
                              .map((user) => (
                                <button
                                  key={user.id}
                                  onClick={() => handleAddUserToSectorForDistrict(user.id)}
                                  disabled={submittingSector}
                                ...
                                >
                                  <div className="text-right">
                                    <div className="font-medium text-sm">{getSectorMemberName(user)}</div>
                                    <div className="text-xs text-gray-500">{user.mobileNumber}</div>
                                  </div>
