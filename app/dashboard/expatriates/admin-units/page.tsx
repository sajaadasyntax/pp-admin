...
interface ExpatriateAdminUnit {
  ...
  description?: string;
  active: boolean;
  expatriateLocalityId: string;
  expatriateLocality?: { id: string; name: string; expatriateRegionId: string; expatriateRegion?: ExpatriateRegion; };
  adminId?: string;
...
export default function ExpatriateAdminUnitsPage() {
  const { token } = useAuth();
  ...
  const [editing, setEditing] = useState<ExpatriateAdminUnit | null>(null);
  const [formData, setFormData] = useState({ name: '', code: '', description: '', expatriateLocalityId: '', active: true });
  const [showAdminModal, setShowAdminModal] = useState(false);
  const [selectedItem, setSelectedItem] = useState<ExpatriateAdminUnit | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await apiCall(`/expatriate-hierarchy/expatriate-admin-units/${editing.id}`, { method: 'PUT', body: JSON.stringify(formData) });
      } else {
        await apiCall('/expatriate-hierarchy/expatriate-admin-units', { method: 'POST', body: JSON.stringify({ ...formData, expatriateLocalityId: selectedLocalityId }) });
      }
      setShowForm(false); setEditing(null); setFormData({ name: '', code: '', description: '', expatriateLocalityId: '', active: true }); fetchAdminUnits();
    } catch (error) { alert('حدث خطأ أثناء الحفظ'); }
    finally { setSubmitting(false); }
  };

  const handleEdit = (item: ExpatriateAdminUnit) => {
    setEditing(item);
    setFormData({ name: item.name, code: item.code || '', description: item.description || '', expatriateLocalityId: item.expatriateLocalityId, active: item.active });
    setShowForm(true);
  };

  const handleDelete = async (id: string) => {
    if (!window.confirm('هل أنت متأكد من حذف هذا العنصر؟')) return;
    try { await apiCall(`/expatriate-hierarchy/expatriate-admin-units/${id}`, { method: 'DELETE' }); fetchAdminUnits(); }
  ...
  const fetchAvailableAdmins = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/users/available-admins?level=expatriate_admin_unit`, { headers: { 'Authorization': `Bearer ${token}` } });
      if (response.ok) { const data = await response.json(); setAvailableAdmins(data.data || []); }
    } catch (error) { console.error('Error:', error); }
    finally { setLoadingAdmins(false); }
  };

  const handleManageAdmin = (item: ExpatriateAdminUnit) => { setSelectedItem(item); setShowAdminModal(true); fetchAvailableAdmins(); };

  const handleAssignAdmin = async (adminId: string | null) => {
    if (!selectedItem) return;
    setSubmitting(true);
    try {
      await apiCall(`/expatriate-hierarchy/expatriate-admin-units/${selectedItem.id}`, { method: 'PUT', body: JSON.stringify({ adminId }) });
      alert(adminId ? 'تم تعيين المسؤول بنجاح' : 'تم إلغاء تعيين المسؤول بنجاح');
      setShowAdminModal(false); fetchAdminUnits();
    } catch (error) { alert('فشل في تعيين المسؤول'); }
    finally { setSubmitting(false); }
  };

  const getAdminName = (item: ExpatriateAdminUnit): string => {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        <Link href="/dashboard/expatriates" className="text-orange-600 hover:text-orange-800 mb-2 inline-block">← العودة للمغتربين</Link>
        <h1 className="text-3xl font-bold text-gray-900">الوحدات الإدارية للمغتربين</h1>
        <p className="text-gray-600">إدارة الوحدات الإدارية في نظام المغتربين</p>
      </div>

      <div className="mb-6 flex flex-wrap items-center gap-4">
        <div>
          <label className="text-sm font-medium text-gray-700 block mb-1">القطاع:</label>
          <select value={selectedRegionId} onChange={(e) => setSelectedRegionId(e.target.value)} className="border rounded-lg px-3 py-2 min-w-[200px]">
            {regions.map((r) => (<option key={r.id} value={r.id}>{r.name}</option>))}
          </select>
        </div>
        <div>
          <label className="text-sm font-medium text-gray-700 block mb-1">المحلية:</label>
          <select value={selectedLocalityId} onChange={(e) => setSelectedLocalityId(e.target.value)} className="border rounded-lg px-3 py-2 min-w-[200px]" disabled={localities.length === 0}>
            {localities.map((l) => (<option key={l.id} value={l.id}>{l.name}</option>))}
          </select>
        </div>
        <button onClick={() => { setShowForm(true); setEditing(null); setFormData({ name: '', code: '', description: '', expatriateLocalityId: selectedLocalityId, active: true }); }}
          className="bg-orange-600 text-white px-4 py-2 rounded-lg hover:bg-orange-700 mr-auto self-end" disabled={!selectedLocalityId}>
          + إضافة وحدة إدارية
        </button>
      </div>
      ...
      {selectedLocality && (
        <div className="mb-4 p-3 bg-orange-50 rounded-lg border border-orange-200">
          <span className="text-orange-800">عرض وحدات المحلية: <strong>{selectedLocality.name}</strong></span>
        </div>
      )}
          ...
          {adminUnits.map((item) => (
            <div key={item.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 hover:shadow-md transition-shadow">
              ...
              <div className="flex items-center justify-between text-sm text-gray-600 mb-3">
                <span>الأحياء: {item._count?.districts || 0}</span>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Link href={`/dashboard/expatriates/districts?adminUnitId=${item.id}`} className="px-3 py-1 bg-red-50 text-red-700 rounded text-xs hover:bg-red-100">الأحياء</Link>
      ...
      {!loading && adminUnits.length === 0 && selectedLocalityId && (
        <div className="text-center py-12 text-gray-500"><p className="text-lg mb-2">لا توجد وحدات إدارية</p><p className="text-sm">أضف وحدة إدارية جديدة للبدء</p></div>
      )}

      {showForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md">
            <h2 className="text-xl font-bold mb-4">{editing ? 'تعديل الوحدة الإدارية' : 'إضافة وحدة إدارية جديدة'}</h2>
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex justify-end gap-2 mt-6">
                ...
                <button type="submit" disabled={submitting} className="px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 disabled:opacity-50">{submitting ? 'جاري الحفظ...' : 'حفظ'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showAdminModal && selectedItem && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md max-h-[80vh] overflow-y-auto">
            ...
            {loadingAdmins ? (<div className="flex justify-center py-8"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-orange-500"></div></div>) : (
